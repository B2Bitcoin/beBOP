...
<main class="mx-auto max-w-7xl py-10 px-6 body-secondPlan">
	<div
		class="w-full rounded-xl body-secondPlan border-gray-300 p-6 grid flex md:grid-cols-3 sm:flex-wrap gap-2"
	>
		<div class="col-span-2 flex flex-col gap-2">
				...
				{#if data.order.payment.method !== 'cash'}
					<ul>
						<li>
							...
							<code class="break-words body-secondaryText break-all">{data.order.payment.address}</code>
						</li>
						<li>
			...
			{#if data.order.shippingAddress}
				<div>
					...
					<pre class="break-words body-secondaryText">{JSON.stringify(data.order.shippingAddress, null, 2)}</pre>
				</div>
			{/if}
